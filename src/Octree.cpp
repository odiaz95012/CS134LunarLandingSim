...
//            sort point data into each box  (see helper function getMeshPointsInBox())
...
void Octree::subdivide(const ofMesh &mesh, TreeNode &node, int numLevels, int level) {
    ...
    // For each child box, sort point data into each box
    for (auto& childBox : childBoxes) {
        vector<int> pointsInBox;
        int pointCount = getMeshPointsInBox(mesh, node.points, childBox, pointsInBox);
        ...
        if (pointCount > 0) {
            ...
            // If child is not a leaf node, recursively call subdivide
            if (pointCount > 1) {
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
    //test for intersection with node box
    if(!node.box.intersect(ray, 0, 10000)) return false;
    if(!node.children.empty()){
        //recursively traverse through following levels, testing for intersection at each level
        ...
        //if we reached a leaf node, this is the selected node
...
//determine what leaf boxes that overlap with input box
...
bool Octree::intersect(Box &box, TreeNode &node, vector<Box> &boxListRtn, vector<TreeNode> &nodeListRtn){
    bool intersects = false;
    if(!node.box.overlap(box)) return false;
    if(!node.children.empty()){
        for(auto& child : node.children){
...
void Octree::draw(TreeNode & node, int numLevels, int level, ofColor colors[], int colorIndex) {
    if (level >= numLevels) return;
    ofSetColor(colors[colorIndex]);
    drawBox(node.box);
    level++;
    for (int i = 0; i < node.children.size(); i++) {
        draw(node.children[i], numLevels, level, colors, colorIndex + 1);
    }
}
